		...
		CheckHit(turf/T)
			...
			if(T.density)
				if(istype(T,/turf/wall))
					var/turf/wall/t=T
					if(((cy < t.pheight) && !t.ceiling) || (t.ceiling && ((py%32)+t.pheight)<cy))// || t.pheight-t.pixel_y>cy)
						return T
			for(var/atom/A in T)
				if(A.density)
